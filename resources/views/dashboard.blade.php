@extends('layouts.app', ['class' => 'dashboard'])
@section('content')
<div class="container-fluid">
    <div class="row">
        <div class="col-sm-12">
            <div class="title-container main">
                <h1 class="head">Dashboard</h1>
            </div>
        </div>
    </div>
    @alertSuccess
    @alertError
    @alertInfo
    {{-- <div class="row">
        <div class="col-sm-12">
            @if (Session::has('account'))
                <div class="">Account Selected: {{Session::get('account')->name}}</div>
            @endif
        </div>
    </div> --}}
    <div class="row">
        <div class="col-sm-3">
            <div class="card card-stats">
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-5">
                            <div class="icon icon-primary">
                                <span class="ti-user"></span>
                            </div>
                        </div>
                        <div class="col-sm-7 text-right">
                            <div class="card-title">
                                Contacts
                            </div>
                            <div class="card-number">
                               {{$account->contacts->count()}}
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card-footer">
                    <a href="{{route('contact.index', ['account' => Session::get('account')->id])}}" class="link">+ Add New Contact</a>
                </div>
            </div>
        </div>
        <div class="col-sm-3">
            <div class="card card-stats">
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-5">
                            <div class="icon icon-info">
                                <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-building" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" d="M14.763.075A.5.5 0 0 1 15 .5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5V14h-1v1.5a.5.5 0 0 1-.5.5h-9a.5.5 0 0 1-.5-.5V10a.5.5 0 0 1 .342-.474L6 7.64V4.5a.5.5 0 0 1 .276-.447l8-4a.5.5 0 0 1 .487.022zM6 8.694L1 10.36V15h5V8.694zM7 15h2v-1.5a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 .5.5V15h2V1.309l-7 3.5V15z"></path>
                                    <path d="M2 11h1v1H2v-1zm2 0h1v1H4v-1zm-2 2h1v1H2v-1zm2 0h1v1H4v-1zm4-4h1v1H8V9zm2 0h1v1h-1V9zm-2 2h1v1H8v-1zm2 0h1v1h-1v-1zm2-2h1v1h-1V9zm0 2h1v1h-1v-1zM8 7h1v1H8V7zm2 0h1v1h-1V7zm2 0h1v1h-1V7zM8 5h1v1H8V5zm2 0h1v1h-1V5zm2 0h1v1h-1V5zm0-2h1v1h-1V3z"></path>
                                </svg>
                            </div>
                        </div>
                        <div class="col-sm-7 text-right">
                            <div class="card-title">
                                Companies
                            </div>
                            <div class="card-number">
                                {{$account->companies->count()}}
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card-footer">
                    <a href="{{route('company.index', ['account' => Session::get('account')->id])}}" class="link">+ Add New Company</a>
                </div>
            </div>
        </div>
    </div> {{-- row --}}
</div> {{-- container-fluid --}}
@endsection
